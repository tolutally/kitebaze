    ...
    <>
      ...
      <section className="bg-kb-canvas px-6 pb-16 text-center sm:pb-40">
        <div className="mx-auto flex max-w-5xl flex-col items-center">
          ...
          <p className="mt-10 max-w-2xl font-space-grotesk text-lg font-light leading-tight text-kb-ink-soft sm:text-xl">Bottleneck is a <span className="text-kb-accent-ink">specific role with a job.</span> Not a chatbot. Not a generic assistant. A worker installed into your day-to-day—so one job runs reliably, without you in the middle.</p>
        </div>
      </section>
      ...
      <section className="relative z-20 mx-auto w-full max-w-7xl bg-kb-canvas px-6 pb-32 pt-16">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">{roles.map(([title, copy], index) => <article key={title} className="kb-card-shadow relative overflow-hidden rounded-[28px] border border-kb-line bg-kb-surface p-8 text-center transition-all hover:border-kb-line-strong"><div className="absolute inset-0 -z-10" style={{ background: `radial-gradient(circle at ${index % 2 ? '100% 0%' : '0% 0%'}, rgba(159,107,78,0.11), transparent 55%)` }}></div><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-full border border-kb-line bg-kb-surface-soft text-xl text-kb-accent-ink shadow-[0_0_15px_rgba(159,107,78,0.16)]">✦</span><h3 className="mt-6 font-inter text-xl font-medium tracking-tight text-kb-ink">{title}</h3><p className="mt-3 font-inter text-sm font-light text-kb-ink-soft">{copy}</p></article>)}</div>
      </section>
      ...
      <section className="relative bg-kb-accent py-32 text-center text-kb-on-accent"><div className="mx-auto max-w-4xl px-4"><h2 className="font-space-grotesk text-5xl font-medium tracking-tight text-kb-on-accent sm:text-7xl">Start with the right role.</h2><p className="mx-auto mb-10 mt-6 max-w-2xl text-lg font-light text-kb-on-accent/80 sm:text-xl">30 minutes. Clear verdict. Next steps if there’s leverage.</p><Link to="/book-workflow-review" className="inline-flex rounded-full bg-kb-inverse px-8 py-3.5 font-inter text-sm text-kb-inverse-text transition-colors hover:bg-kb-inverse-soft">Book a Workflow Review</Link></div></section>
    </>
  );
}

function CallButton() { return <Link to="/book-workflow-review" className="inline-flex w-fit items-center justify-center rounded-full bg-kb-accent px-8 py-3.5 font-inter text-sm font-medium text-kb-on-accent transition-colors hover:bg-kb-accent-hover">Book a Workflow Review</Link>; }
